...
`gawee`는 PyTorch로 작성된 딥러닝 모델을 ONNX로 변환한 뒤,  
ONNX 기반 **자체 IR(Intermediate Representation)** 위에서  
**그래프 분석 및 최적화(graph optimization)** 를 수행하는  
toy-level 딥러닝 컴파일러 프론트엔드 프로젝트입니다.

본 프로젝트의 목적은 **실무 딥러닝 컴파일러(예: TVM 계열)가 실제로 수행하는 문제를  
최소한의 스코프로 직접 설계·구현·검증하는 것**입니다.
...
- ONNX를 입력 포맷으로 사용하는 **분석 친화적 자체 IR 설계**
- 그래프 레벨 최적화의 원리와 정합성(soundness) 이해 및 구현
- 최적화 전/후 그래프의 **FLOPs 및 runtime 비교를 통한 효과 검증**
...
- PyTorch의 동적 Python 코드가 **정적 계산 그래프**로 변환되는 경계 지점 이해
- `torch.onnx.export`를 이용해 ResNet-18을 ONNX 모델로 export
- ONNX graph의 구성 요소:
  - Node (op type, attributes)
  - Tensor (shape, dtype)
...
ONNX를 그대로 최적화 대상으로 사용하지 않고,  
**그래프 분석과 최적화를 위해 필요한 정보만을 명시적으로 표현하는 자체 IR**를 정의한다.

IR 설계의 주요 관심사:

- 연산 노드와 데이터 흐름의 명확한 분리
- shape / dtype / layout 정보의 명시적 표현
...
본 프로젝트는 **프론트엔드 및 그래프 레벨 최적화**에 한정
